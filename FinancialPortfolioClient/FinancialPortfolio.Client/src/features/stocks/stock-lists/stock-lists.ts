...
const LOG_FILE = 'stock-lists.ts';

@Component({
  selector: 'app-stock-lists',
  standalone: true,
  imports: [CommonModule, DataGrid, PageHeader],
  templateUrl: './stock-lists.html',
  styleUrl: './stock-lists.css',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class StockLists implements OnInit {
  ...
  readonly headerActions = computed<PageHeaderAction[]>(() => [
    ...
    readonly columns = signal<TableColumn<Stock>[]>([
    {
      key: 'symbol',
      header: 'Symbol',
      sortable: true,
  ...
  onColumnsChange(cols: TableColumn<Stock>[]): void {
  ...
  async forceRefresh(): Promise<void> {
    const ok = await this.confirm.open({
      title: 'Force Refresh Stocks',
      message: 'Sync latest stock data from Google Sheet? This may take a moment.',
  ...
  load(): void {
    ...
    this.stockService.search(this.query()).subscribe({
      next: (result) => {
        this.stocks.set(result?.data ?? []);
      ...
      error: (err) => {
        ...
        this.logger.error('Failed to load stocks', err, LOG_FILE, 'load');
      },
    });
  }

  viewStock(row: Stock): void {
    void this.router.navigate(['/stocks', row.id]);
  }
